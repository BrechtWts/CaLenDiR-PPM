...
def train_eval(log_name, 
    ...
    clen_dis_ref : bool, optional
        ...
        Case-Based Sampling (UCBS) of instances each epoch. In contrast 
        to CRTP-, ED-LSTM and SuTraN, CaLenDiR training for SEP-LSTM does 
        not include Suffix-Length-Normalized Loss Functions, since 
        SEP-LSTM is only trained for one-step-ahead predictions, and 
        hence both the activity and timestamp prediction loss functions 
        are computed over a single prediction per instance, rather than 
        suffixes of predictions per instance. If `False`, the default 
        training procedure, in which all instances are used for training 
        each epoch (and hence in which case-length distortion is not 
        addressed), is performed. 
        `True` by default. Note that the default (non-CaLenDiR) training 
    ...
    subfolder_path = os.path.join(log_name, "SEP_LSTM_results")
    ...
    # if clen_dis_ref:
    #     _, inf_results, pref_suf_results = inference_loop(model, 
    #                                                       test_dataset, 
    #                                                       mean_std_ttne, 
    #                                                       mean_std_tss_pref, 
    #                                                       mean_std_tsp_pref, 
    ...
    #                                                       val_batch_size=2048)
    # else:
    #     inf_results, _, pref_suf_results = inference_loop(model, 
    #                                                       test_dataset, 
    #                                                       mean_std_ttne, 
    #                                                       mean_std_tss_pref, 
    #                                                       mean_std_tsp_pref, 
    ...
    avg_MAE_ttne_stand_IB, avg_MAE_ttne_seconds_IB, avg_MAE_ttne_minutes_IB, avg_dam_lev_IB, avg_MAE_rrt_sum_minutes_IB = inf_results_IB
    ...
    avg_MAE_ttne_stand_CB, avg_MAE_ttne_seconds_CB, avg_MAE_ttne_minutes_CB, avg_dam_lev_CB, avg_MAE_rrt_sum_minutes_CB = inf_results_CB
