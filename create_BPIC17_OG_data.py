...
def construct_BPIC17_datasets():
    df = pd.read_csv(r'bpic17_with_loops.csv', header = 0)
    df = preprocess_bpic17(df)

    # NOTE: 'lifecycle:transition' not a case feature but an event feature. Does not make 
    # a difference in the preprocessing however. 'mistake' retained such that the same 
    # tensors (with the same order) can be retained compared to the experiments reported on 
    # in the paper. 
    ...
    train_data, val_data, test_data, og_caseint_train, og_caseint_val, og_caseint_test = result
