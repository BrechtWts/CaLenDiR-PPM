...
def generate_tensordata(pref_suff, 
                        case_id, 
                        act_label, 
                        cardinality_dict, 
                        num_cols_dict, 
    ...
    set_type : {'train', 'val', 'test'}
        The dataset for which the tensors are created. Either `'train'`, 
        `'val'` (validation set) or `'test'`.
    ...
    # Generate a tensor of shape (num_prefs,) and dtype torch.int64 that 
    # contains an integer mapping of the original case ids from which each 
    # instance is derived. 
    #   - Retaining only first row for each instance 
    pref_df_case = prefix_df.drop_duplicates(subset=case_id).copy()
    #   - Deriving a list of the unique original case IDs 
    orig_case_list = list(pref_df_case['orig_case_id'].unique())

    #   - Creating integer mapping list 
    orig_case_intmap = [i for i in range(len(orig_case_list))]

    #   - Create mapping dictionary 
    intmap_orig = dict(zip(orig_case_list, orig_case_intmap))

    #   - Creating integer mapped column 
    pref_df_case['int_og_id'] = pref_df_case['orig_case_id'].map(intmap_orig)

    #   - constructing final tensor 
    og_caseint_tensor = torch.from_numpy(pref_df_case['int_og_id'].to_numpy()).to(torch.int64) # (num_prefs, )
    ...
    # Writing the case ID mapping to disk, such that the original case 
    # IDs can be reconstructed afterwards. 
    # Saving everything to disk
    output_directory = log_name
    os.makedirs(output_directory, exist_ok=True)

    intmap_ogID_path = os.path.join(output_directory, set_type + '_intmap_orig.pkl')
    with open(intmap_ogID_path, 'wb') as file:
        pickle.dump(intmap_orig, file)

    return all_tensors, og_caseint_tensor
...
def generate_tensordata_train_test(train_pref_suff, 
    ...
    train_data, og_caseint_train = generate_tensordata(pref_suff = train_pref_suff, case_id = case_id, 
    ...
    val_data, og_caseint_val = generate_tensordata(pref_suff = val_pref_suff, case_id = case_id, 
                                     act_label = act_label, cardinality_dict = cardinality_dict, 
                                     num_cols_dict = num_cols_dict, 
                                     cat_cols_dict = cat_cols_dict, 
                                     window_size = window_size, 
    ...
    test_data, og_caseint_test = generate_tensordata(pref_suff=test_pref_suff, case_id = case_id, 
                                    act_label = act_label, cardinality_dict = cardinality_dict, 
                                    num_cols_dict = num_cols_dict, 
                                    cat_cols_dict = cat_cols_dict, 
                                    window_size = window_size, 
    ...
    return train_data, val_data, test_data, num_pref_cat, num_suff_cat, pref_cat_cars, suff_cat_cars, num_activities, og_caseint_train, og_caseint_val, og_caseint_test
