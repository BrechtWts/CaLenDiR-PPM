...
def inference_loop(model, 
    ...
    * `num_activities` : the total number of possible activity labels 
    ...
    with torch.no_grad():
        ...
        # Binary outcome prediction (Future work)
        if outcome_bool:
            avg_BCE_out_CB = compute_corrected_avg(metric_tens=inference_BCE, 
                                                   weight_tens=weights, 
                                                   num_cases=num_cases)
            # To do for future work with binary outcome prediction: 
            # Implementing CB metrics for auc_roc and auc_pr 
    ...
    if outcome_bool:
         return_list_IB += [avg_BCE_out, auc_roc, auc_pr]
    ...
    # Future work - NOTE: CB variants for auc roc and auc pr should still 
    #                     be implemented. 
    if outcome_bool:
         return_list_CB += [avg_BCE_out_CB, auc_roc, auc_pr]
