...
def inference_loop(model, 
    ...
    * `num_activities` : the total number of possible activity labels 
    ...
    with torch.no_grad():
        ...
        # CB average MAE (both in seconds and in minutes)
        # Seconds 
        avg_MAE_seconds_RRT_CB = compute_corrected_avg(metric_tens=MAE_rrt_seconds, 
                                                        weight_tens=weights, 
                                                        num_cases=num_cases) # scalar 
        # Minutes 
        avg_MAE_minutes_RRT_CB = avg_MAE_seconds_RRT_CB / 60 # scalar 
        ...
        # Metrics underneath would only serve for informational purposes, 
        # without being reported on. Omitted for this paper. 
        # # Length differences: 
    ...
    return_list_IB= [avg_MAE_ttne_stand, avg_MAE_ttne_minutes, avg_dam_lev, avg_MAE_minutes_RRT]
    ...
    return_list_CB = [avg_MAE_ttne_stand_CB, avg_MAE_ttne_minutes_CB, avg_dam_lev_CB, avg_MAE_minutes_RRT_CB]
